...
// GetBackend returns the configured storage backend. Empty and legacy metadata
// values resolve to Dolt; every explicit backend name is normalized and
// preserved so built-in SQL backends and external backend plugins share one
// configuration path.
