...
func OpenSQL(ctx context.Context, dir, database, branch string) (*sql.DB, func() error, error) {
	dbPath, dbFile, err := buildDSN(dir, database)
	if err != nil {
		return nil, nil, err
	}
...
func buildDSN(dir, database string) (string, string, error) {
	...
	return fmt.Sprintf("%s?_busy_timeout=%d", path, defaultBusyTimeout), path, nil
}
