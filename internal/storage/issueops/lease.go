...
func ReclaimExpiredLeasesInTx(ctx context.Context, tx DBTX, cutoff time.Time, actor string) ([]types.ReclaimedLease, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.PreviousOwner); err != nil {
			if closeErr := rows.Close(); closeErr != nil {
				return nil, fmt.Errorf("scan stale lease row: %w; close stale lease rows: %v", err, closeErr)
			}
	...
	if err := rows.Err(); err != nil {
		if closeErr := rows.Close(); closeErr != nil {
			return nil, fmt.Errorf("iterate stale leases: %w; close stale lease rows: %v", err, closeErr)
		}
