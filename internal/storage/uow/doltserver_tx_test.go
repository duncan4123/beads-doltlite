...
//
// Re-landed on spike/backend-seam-derisk: the hardening (commit 794ff0790) was
// reverted to BASE in a59e75325's serverv2 triage, which left doltServerTx.Commit
// releasing the pinned session with its transaction still open on a non-transient
// DOLT_COMMIT failure — the exact late/double-apply hazard SPIKE-REPORT §8.2 asserts
// is handled and RunInTransaction (internal/storage/uowstore/tx.go) now makes a
// first-class adapter path. go-sql-driver v1.9.3 ResetSession only does a liveness
